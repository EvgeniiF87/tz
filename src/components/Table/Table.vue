<template>
	<table>
		...
		<tbody>
			...
			<tr v-else>
				<td colspan="4" class="not-user">Пользователь(-и) не найден(-ы)</td>
			</tr>
		</tbody>
	</table>
</template>
...
export default {
	...
	computed: {
		...
		filteredLoginAndStatusUser() {
			...
			const comfirmed_orders_from =
				!this.$route.query.comfirmed_orders_from ||
				this.$route.query.comfirmed_orders_from === ""
					? 0
					: this.$route.query.comfirmed_orders_from
			const comfirmed_orders_to =
				!this.$route.query.comfirmed_orders_to ||
				this.$route.query.comfirmed_orders_to === ""
					? this.users.map((user) => Math.max(user.comfirmedOrders))[0] + 1
					: this.$route.query.comfirmed_orders_to
			...
			return [...this.users]
				.filter((user) => {
					if (status === "все") {
						return (
							user.login.includes(this.searchValue) &&
							user.comfirmedOrders >= +comfirmed_orders_from &&
							user.comfirmedOrders <= +comfirmed_orders_to
						)
					} else {
						return (
							user.login.includes(this.searchValue) &&
